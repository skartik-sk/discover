...
export async function generateMetadata({
  ...
  try {
    ...
    if (username === "demo") {
      ...
      const categoryData = project.category_id
        ? await supabase
            .from("categories")
            .select("name")
    ...
    const tagNames = tagsResult.data?.map((tag: any) => tag.tag_name) || [];
  ...
  try {
    ...
    if (username === "demo") {
      ...
      try {
        ...
        // Fetch user data for reviews if we have reviews
        let reviewsWithUsers = [];
        if (reviewsResult.data && reviewsResult.data.length > 0) {
          const userIds = [
            ...new Set(
              reviewsResult.data.map((r: any) => r.creator_id).filter(Boolean),
            ),
          ];
          ...
          reviewsWithUsers = reviewsResult.data.map((review: any) => ({
            ...review,
            users: usersMap.get(review.creator_id) || null,
          }));
        }
    ...
    // Fetch related data separately
    const [categoryResult, tagsResult, reviewsResult] = await Promise.all([
      project.category_id
        ? supabase
            .from("categories")
            .select("id, name, slug, color, gradient")
